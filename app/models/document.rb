class Document < ActiveRecord::Base
  ...
  belongs_to :user
  belongs_to :job
  has_attached_file :upload, :styles => { :medium => "1024x683", :thumb => "200x200" }, :acl => :public_read
  ...
  def self.finalize_and_cleanup(id)
    ...
    if document.post_process_required?
      ...
      paperclip_file_path = "documents/uploads/#{id}/#{direct_upload_url_data[:filename]}"
