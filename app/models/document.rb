class Document < ActiveRecord::Base
  ...
  DIRECT_UPLOAD_URL_FORMAT = %r{\Ahttps:\/\/s3\.amazonaws\.com\/jobsuploadsdev#{!Rails.env.development? ? "\\-#{Rails.env}" : ''}\/(?<path>uploads\/.+\/(?<filename>.+))\z}.freeze
  
  belongs_to :user
  belongs_to :job
  has_attached_file :upload, :styles => { :medium => "300x300>", :thumb => "100x100>" }, :acl => :public_read
